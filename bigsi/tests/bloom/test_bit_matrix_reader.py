...
def test_bit_matrix_reader_creation_success(num_rows, num_cols):
...
def test_bit_matrix_reader_creation_failure(num_rows, num_cols):
...
def test_bit_matrix_reader_iteration_success(num_cols, byte_values):
...
@given(num_cols=st.integers(min_value=1, max_value=8),
       byte_values=st.lists(min_size=1, max_size=100, elements=st.integers(min_value=0, max_value=255)))
def test_bit_matrix_reader_raise_exception_past_iteration(num_cols, byte_values):
    num_rows = math.floor(len(byte_values) * 8 / num_cols)
